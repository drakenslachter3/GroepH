        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('smartmeters.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-600 text-white rounded-md shadow-sm flex items-center">
                        ...
                        <tbody>
                            @forelse ($smartMeters as $meter)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="py-4 px-6">
                                        @if ($meter->user)
                                            <a href="{{ route('users.show', $meter->user->id) }}" class="text-blue-600 hover:underline">
                                                {{ $meter->user->name }}
                                            </a>
                                        @else
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="flex space-x-2">
                                            ...
                                            <a href="{{ route('smartmeters.edit', $meter->id) }}" class="px-3 py-1 text-yellow-500 hover:text-yellow-700">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <form method="POST" action="{{ route('smartmeters.destroy', $meter->id) }}" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Weet je zeker dat je deze slimme meter wilt verwijderen?');" class="px-3 py-1 text-red-500 hover:text-red-700">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
