<div x-data="{
    ...
    <button x-ref="button" x-on:click="toggle()" :aria-expanded="open" :aria-controls="$id('dropdown-button')"
        type="button"
        class="notification-dropdown-toggle relative p-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none">
        ...
        @if ($unreadCount > 0)
            <span
                class="notification-badge absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">
                {{ $unreadCount }}
            </span>
        @endif
    </button>
            ...
            @else
                <div class="max-h-80 overflow-y-auto">
                    @foreach ($unreadNotifications as $notification)
                        <div
                            class="p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md mb-2 border-l-4 border-{{ $notification->getSeverityColorClass() }}-500">
                            <div class="flex justify-between">
                                <div class="w-full">
                                    <div class="flex items-center">
                                        <span
                                            class="px-2 py-0.5 text-xs font-medium rounded-full bg-{{ $notification->getSeverityColorClass() }}-100 text-{{ $notification->getSeverityColorClass() }}-800 dark:bg-{{ $notification->getSeverityColorClass() }}-900/30 dark:text-{{ $notification->getSeverityColorClass() }}-300 mr-2">
