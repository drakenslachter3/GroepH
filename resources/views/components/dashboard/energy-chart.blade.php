...
@php
    ...
    $currentPeriodLabel = match ($period) {
        ...
        'month' => $currentDate->isSameMonth(Carbon::now())
            ? 'Totaal deze maand'
            : 'Totaal in ' . $currentDate->translatedFormat('F Y'),
        'year' => $currentDate->isSameYear(Carbon::now())
            ? 'Totaal dit jaar'
            : 'Totaal in ' . $currentDate->format('Y'),
        default => 'Totaal',
    };
...
<section class="p-2" aria-labelledby="chart-widget-title">
    <div aria-label="{{ __('energy-chart-widget.navigation_label') }}"
        ...
        <div role="group" aria-label="{{ __('energy-chart-widget.period_selection') }}"
            class="flex w-full sm:w-auto mt-2 sm:mt-0 overflow-hidden rounded-md">
    ...
    <div class="flex justify-between items-center mb-4">
        ...
        {{-- Label --}}
        <span
            class="px-3 py-1 text-sm bg-{{ $buttonColor }}-500 text-white dark:bg-{{ $buttonColor }}-600 dark:text-white rounded-md"
    ...
    <div class="mt-4 flex flex-col gap-2 text-sm text-gray-800 dark:text-gray-100">
        ...
        <div id="previous-year-total-{{ $type }}"
            class="flex items-center justify-between transition-all duration-300 opacity-0 h-6 pointer-events-none">
    ...
    <div class="mt-4 flex flex-col sm:flex-row justify-between items-center gap-2">
        ...
        <div id="comparison-date-form-wrapper-{{ $type }}" class="{{ $comparisonActive ? '' : 'hidden' }}">
            <form id="comparison-date-form-{{ $type }}" method="GET" action="{{ route('dashboard') }}"
                ...
                <input id="comparison-date-{{ $type }}" name="comparison_date" type="{{ $inputType }}"
                    min="{{ $period === 'year' ? '2020' : $minDate }}"
                    max="{{ $period === 'year' ? now()->year - 1 : $maxDate }}" step="1"
                    ...
                    style="min-width: 110px;" value="{{ $defaultComparison }}"
    ...
    <div class="sr-only focus-within:not-sr-only focus:not-sr-only">
        <h2 tabindex="0" class="text-lg font-semibold mb-2 dark:text-white" id="{{ $type }}TableCaption">
            ...
            @endphp
            {{ $title }} -
            {{ __('energy-chart-widget.consumption_overview', ['period' => $formattedPeriodDate]) }}
        </h2>

        <div class="overflow-x-auto">
            <table class="w-full border-collapse table-auto" role="table"
                ...
                <thead>
                    <tr role="row">
                        <th scope="col" role="columnheader" tabindex="0"
                            ...
                            {{ __('energy-chart-widget.period_column_' . $period) }} </th>
                        <th scope="col" role="columnheader" tabindex="0"
                            class="border p-2 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 text-left font-semibold">
                ...
                <tbody role="rowgroup">
                    ...
                    @foreach ($currentData as $index => $value)
                        @php
                            ...
                            $percentChange =
                                $prevValue && $prevValue != 0 ? (($value - $prevValue) / $prevValue) * 100 : null;

                            switch ($period) {
                                case 'day':
                                    $hour = str_pad($index, 2, '0', STR_PAD_LEFT);
                                    $nextHour = str_pad($index + 1, 2, '0', STR_PAD_LEFT);
                                    $dateFormat = __('energy-chart-widget.between_hours', [
                                        'start' => $hour,
                                        'end' => $nextHour,
                                    ]);
                                    break;
                                ...
                                case 'year':
                                    $months = [
                                        __('energy-chart-widget.months.january'),
                                        __('energy-chart-widget.months.february'),
                                        __('energy-chart-widget.months.march'),
                                        __('energy-chart-widget.months.april'),
                                        __('energy-chart-widget.months.may'),
                                        __('energy-chart-widget.months.june'),
                                        __('energy-chart-widget.months.july'),
                                        __('energy-chart-widget.months.august'),
                                        __('energy-chart-widget.months.september'),
                                        __('energy-chart-widget.months.october'),
                                        __('energy-chart-widget.months.november'),
                                        __('energy-chart-widget.months.december'),
                                    ];
                                    $monthName =
                                        $months[$index] ??
                                        __('energy-chart-widget.month_number', ['number' => $index + 1]);
                                    $dateFormat = __('energy-chart-widget.in_month', ['month' => $monthName]);
                                    break;

                                default:
                                    $dateFormat = __('energy-chart-widget.consumption_for_interval', [
                                        'index' => $index,
                                    ]);
                                    break;
                            }
                        @endphp
                        <tr role="row">
                            <td role="gridcell" tabindex="0" class="border p-2 dark:border-gray-700"
                                aria-describedby="{{ $type }}TableDescription">
                                {{ $dateFormat }}
                            </td>
                            <td role="gridcell" tabindex="0" class="border p-2 dark:border-gray-700 font-mono"
                            ...
                            <td role="gridcell" tabindex="0"
                                ...
                                @if ($diff !== null)
                                    <span
                                        class="{{ $diff < 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                    ...
                    <tr role="row" class="bg-gray-50 dark:bg-gray-800">
                        ...
                        <td role="gridcell" tabindex="0"
                            ...
                            @if ($hasPreviousYearData)
                                ...
                                <span
                                    class="{{ $totalDiff < 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
        ...
        <div class="mt-4 not-sr-only">
            <button type="button" onclick="scrollAndFocus('{{ $type }}TableCaption')" tabindex="0"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function hasOutage(index, period, date) {
                ...
                switch (period) {
                    ...
                    case 'month':
                        ...
                        periodStart = new Date(currentDate.getFullYear(), currentDate.getMonth(), index + 1, 0, 0,
                            0);
                        periodEnd = new Date(currentDate.getFullYear(), currentDate.getMonth(), index + 1, 23, 59,
                            59);
                        break;
                    case 'year':
                ...
                return outagesData.some(outage => {
                    ...
                    const outageEnd = outage.end_time ? new Date(outage.end_time) :
            ...
            switch (period) {
                ...
                case 'month':
                    ...
                    break;
                case 'year':
                    labels.push("{{ __('energy-chart-widget.months.january') }}",
                        "{{ __('energy-chart-widget.months.february') }}",
                        "{{ __('energy-chart-widget.months.march') }}",
                        "{{ __('energy-chart-widget.months.april') }}",
                        "{{ __('energy-chart-widget.months.may') }}",
                        ...
                        outageZones.forEach(zone => {
                            if (zone.hasOutage) {
                                ...
                                if (bar) {
                                    ...
                                    ctx.save();
                                    ctx.fillStyle =
                                    'rgba(249, 115, 22, 0.2)'; // Light orange background
                                    ctx.fillRect(x, chartArea.top, barWidth, chartArea
            ...
            const previousYearComparisons = document.querySelectorAll(
                '.previous-year-comparison-{{ $type }}');
            ...
            if (toggleButton) {
                toggleButton.addEventListener('click', function() {
                    ...
                    const formWrapper = document.getElementById(
                        'comparison-date-form-wrapper-{{ $type }}');
