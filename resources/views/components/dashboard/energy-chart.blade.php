...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
    <div class="p-6 bg-white border-b border-gray-200">
        <div class="flex justify-between items-start mb-4">
            ...
            <div>
                <button id="toggle{{ ucfirst($type) }}Comparison" class="text-sm px-3 py-1 bg-{{ $buttonColor }}-100 text-{{ $buttonColor }}-700 rounded hover:bg-{{ $buttonColor }}-200">
                    {{ $buttonLabel }}
                </button>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const {{ $type }}Ctx = document.getElementById('{{ $type }}Chart').getContext('2d');
        const {{ $type }}Chart = new Chart({{ $type }}Ctx, {
            type: 'bar',
            data: {
                    ...
                    tooltip: {
                        callbacks: {
                            afterBody: function(context) {
                                ...
                                const value = chartData.{{ $type }}.data[dataIndex];
                                const target = chartData.{{ $type }}.target[dataIndex];
        ...
        document.getElementById('toggle{{ ucfirst($type) }}Comparison').addEventListener('click', function() {
            const button = this;
