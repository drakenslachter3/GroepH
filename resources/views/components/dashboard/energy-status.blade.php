...
<div class="p-4">
    <div class="flex justify-between items-start mb-4">
        <div>
            ...
            @if (isset($date) && isset($period))
                <div
                    class="mt-1 inline-block bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-md text-xs font-medium text-gray-700 dark:text-gray-300">
                    @switch($period)
                        @case('day')
        ...
        <div class="tooltip relative">
            <svg xmlns="http://www.w3.org/2000/svg"
                class="h-5 w-5 text-gray-400 cursor-pointer hover:text-gray-600 dark:text-gray-300 dark:hover:text-white"
                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span
                class="tooltiptext invisible absolute z-10 px-3 py-2 text-sm bg-gray-800 text-white rounded shadow-lg -right-4 -bottom-20 w-48">
                Dit toont uw {{ strtolower($type) }}verbruik ten opzichte van uw budget. Een lager percentage is beter
                voor het milieu.
            </span>
        </div>
    </div>
    ...
    <div class="mt-4">
        ...
        <div class="w-full h-4 bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden relative">
            ...
            <div class="absolute h-full transition-all duration-1000 ease-out bg-green-500"
                style="width: {{ $greenZoneWidth }}%; left: 0;"></div>

            <!-- Rode deel (loopt vanaf zwarte streep tot 100%) -->
            @if ($actualPercentage > 100)
                <div class="absolute h-full transition-all duration-1000 ease-out bg-red-600"
                    style="width: {{ $redZoneWidth }}%; left: {{ $greenZoneWidth }}%;"></div>
            @endif

            <!-- Zwarte streep op positie -->
            <div class="absolute top-0 bottom-0 w-1 bg-black z-10 transition-all duration-1000 ease-out"
                style="left: {{ $dividerPosition }}%;"></div>
            ...
            @if ($actualPercentage > 100)
                ...
                <div class="absolute top-0 right-0 h-full flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-red-600" viewBox="0 0 20 20"
                        fill="currentColor">
        ...
        <div class="flex justify-between mt-1 relative">
            ...
            @if ($actualPercentage > 100)
                <span
                    class="text-xs font-medium absolute 
                        {{ $actualStatus === 'goed'
        ...
        <div class="mt-2 text-xs">
            @if ($actualPercentage < 80)
                <span class="text-green-600 dark:text-green-400">Uitstekend! Je verbruik ligt ruim onder je
                    target.</span>
    ...
    <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 {{ $color }} mr-1" viewBox="0 0 20 20"
                    fill="currentColor">
                ...
                <span class="{{ $color }} text-xs">{{ abs($reductionPercent) }}%
                    {{ $reductionPercent > 0 ? 'vermindering' : 'toename' }} vergeleken met vorig jaar</span>
            </div>
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ number_format($previousYearValue, 2) }}
                {{ $unit }}</span>
        </div>
    </div>
</div>
