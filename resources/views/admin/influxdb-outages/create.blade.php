<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Nieuwe InfluxDB Uitval Toevoegen</h1>
            <a href="{{ route('admin.influxdb-outages.index') }}" class="text-blue-600 hover:text-blue-800 text-sm">←
                Terug naar overzicht</a>
        </div>

        <div class="bg-white shadow-md rounded-lg p-6">
            <form action="{{ route('admin.influxdb-outages.store') }}" method="POST">
                @csrf
                ...
                <div class="mt-6">
                    ...
                    <textarea id="description" name="description" rows="4"
                        ...
                        placeholder="Optionele beschrijving van de uitval...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
